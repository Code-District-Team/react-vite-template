import * as ServerSideFilters from "./ServerSideAntd.stories";
...
The `ServerSideAntd` component is a React component that demonstrates how to create a server-side data table using Ant Design.
It includes features like searching, sorting, and filtering data.

## Features

The `ServerSideAntd` offers the following features:
...
The code for the ServerSideAntd component is organized into different sections, including:
...
To use the `ServerSideAntd`, you can follow these steps:
...
3. Declare an array of object named `columns` for Ant Design table columns and create methods for rendering filter dropdown.
...
4. Create a sperate React component for action buttons. These buttons will be used in filter dropdown that were create in above step.
...
6. In the final step add return statements with following jsx code in `ServerSideAntd` component.
