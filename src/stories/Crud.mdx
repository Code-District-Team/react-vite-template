import * as Crud from "./Crud.stories";
...
The `ProductsCRUD` is a React component that provides a Create, Read, Update, and Delete (CRUD) interface for managing a list of products. It utilizes Ant Design components and React to create a user-friendly product management system.

## Features

The `ProductsCRUD` offers the following features:
...
The code for the CRUDComponent is organized into different sections, including:
...
- The main CRUDComponent component, which manages the list of products, including fetching, creating, updating, and deleting products.

## Usage

To use the `ProductsCRUD`, you can follow these steps:
...
5. Add following methods in the `ProductsCRUD`
...
6. Create a new file named `ProductFormModal.jsx` and paste the following code in it.

The `ProductFormModal` component is a React modal that serves as a form for creating and editing product information.
It allows users to input the product's name, quantity, and price.

### Props

The ProductFormModal component accepts the following props:
...
7. In the final step add return statements with following jsx code in `ProductsCRUD` component.
